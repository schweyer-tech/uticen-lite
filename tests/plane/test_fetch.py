...
def test_json_array_becomes_csv():
    ...
    snap = fetch.fetch_snapshot("https://x/items.json", opener=_opener(body, "application/json"))
    assert snap.fmt == "csv"
...
def test_record_path_navigates_and_headers_forwarded():
    ...
    snap = fetch.fetch_snapshot(
        "https://x/api", headers={"Authorization": "Bearer t"}, record_path="data.items", opener=op
    )
...
def test_csv_passthrough():
    snap = fetch.fetch_snapshot("https://x/data.csv", opener=_opener(b"id,n\nA,1\n", "text/csv"))
...
def test_errors_are_fetcherror():
    ...
    with pytest.raises(fetch.FetchError):  # not a JSON array
        fetch.fetch_snapshot("https://x/o.json", opener=_opener(b'{"a":1}', "application/json"))

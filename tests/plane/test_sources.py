...
def test_save_source_metadata_persists(client):
    ...
    client.post(
        "/sources",
        data={"source_id": "invoices", "format": "csv", "as_of_date": "2026-03-31"},
    ...
    resp = client.post(
        ...
        data={
            ...
            "display_name__invoice_id": "Invoice ID",
            "data_type__invoice_id": "text",
            "is_key__invoice_id": "on",
...
def test_refresh_same_columns_archives_and_preserves_mapping(client):
    ...
    resp = client.post(
        "/sources/tx/refresh/confirm", data={"pending": "tx.csv"}, follow_redirects=False
    )
    assert resp.status_code in (302, 303)
...
def test_refresh_diff_columns_reconciles_after_confirm(client):
    ...
    client.post(
        "/sources/acc",
        data={
            "display_name__acct_id": "Account",
            "data_type__acct_id": "text",
            "include__acct_id": "on",
            "display_name__balance": "Balance",
            "data_type__balance": "number",
            "include__balance": "on",
        },
        follow_redirects=False,
    )
    ...
    client.post("/sources/acc/refresh/confirm", data={"pending": "acc.csv"}, follow_redirects=False)
...
def test_refresh_cancel_discards_pending_and_keeps_current(client):
    ...
    client.post(
        "/sources/keep/refresh",
        files={"file": ("keep.csv", io.BytesIO(b"a\n2\n"), "text/csv")},
        follow_redirects=False,
    )
    resp = client.post(
        "/sources/keep/refresh/cancel", data={"pending": "keep.csv"}, follow_redirects=False
    )
    assert resp.status_code in (302, 303)
    root = client.app.state.project_root
    assert (root / "data" / "keep.csv").read_bytes() == original  # untouched
    assert (
        not list((root / "data" / ".versions" / "keep").glob("*"))
        if (root / "data" / ".versions" / "keep").is_dir()
        else True
    )  # nothing archived
...
def test_create_records_current_file_with_asof(client):
    client.post(
        "/sources",
        data={"source_id": "inv", "format": "csv", "as_of_date": "2026-05-01"},
...
def test_data_tab_preview_paging_and_count(client):
    ...
    client.post(
        "/sources",
        data={"source_id": "big", "format": "csv", "as_of_date": "2026-01-01"},
...
def test_data_tab_asof_edit_syncs(client):
    client.post(
        "/sources",
        data={"source_id": "z", "format": "csv", "as_of_date": "2026-01-01"},
        files={"file": ("z.csv", io.BytesIO(b"a\n1\n"), "text/csv")},
        follow_redirects=False,
    )
    client.post("/sources/z/data/asof", data={"as_of_date": "2026-07-07"}, follow_redirects=False)
    from uticen_lite.store import repo
    from uticen_lite.store.db import connect
...
def test_history_tab_lists_versions(client):
    client.post(
        "/sources",
        data={"source_id": "h", "format": "csv", "as_of_date": "2026-02-02"},
...
def test_data_tab_flags_failed_coercion(client):
    ...
    client.post(
        "/sources",
        data={"source_id": "bad", "format": "csv", "as_of_date": "2026-01-01"},
...
def test_data_tab_no_card_for_clean_numeric(client):
    ...
    client.post(
        "/sources",
        data={"source_id": "ok", "format": "csv", "as_of_date": "2026-01-01"},
...
def test_data_tab_empty_numeric_column_not_flagged(client):
    ...
    client.post(
        "/sources",
        data={"source_id": "empt", "format": "csv", "as_of_date": "2026-01-01"},
...
def test_blank_title_clears_to_none(client):
    ...
    client.post(
        "/sources",
        data={"source_id": "s", "format": "csv"},
        files={"file": ("s.csv", io.BytesIO(csv), "text/csv")},
        follow_redirects=False,
    )
    client.post(
        "/sources/s",
        data={"title": "   ", "display_name__a": "A", "data_type__a": "text", "include__a": "on"},
        follow_redirects=False,
    )
    from uticen_lite.store import repo
    from uticen_lite.store.db import connect

...
def test_upload_parquet(client):
    ...
    resp = client.post(
        ...
        files={"file": ("p.parquet", io.BytesIO(buf.getvalue()), "application/octet-stream")},
        follow_redirects=False,
    )
    assert resp.status_code in (302, 303)
...
def test_xlsx_sheet_selection_persisted(client):
    raw = _xlsx({"First": pd.DataFrame({"id": ["A"]}), "Second": pd.DataFrame({"id": ["Z"]})})
    client.post(
        "/sources",
...
def test_data_preview_degrades_when_adapters_unavailable(client, monkeypatch):
    ...
    msg = (
        "Excel/Parquet support needs the optional dependencies: pip install 'uticen-lite[adapters]'"
    )
...
def test_confirm_refresh_preserves_sheet(client, tmp_path):
    ...
    client.post(
        "/sources",
        data={"source_id": "refresh_sheet", "as_of_date": "2026-01-01", "sheet": "Second"},
        files={
            "file": (
                "rs.xlsx",
                io.BytesIO(raw),
    ...
    # Confirm the refresh
    client.post(
        "/sources/refresh_sheet/refresh/confirm",
...
def test_save_source_preserves_sheet(client):
    ...
    raw = _xlsx({"Main": pd.DataFrame({"val": [1]}), "Alt": pd.DataFrame({"val": [2]})})
    client.post(
        "/sources",
    ...
    client.post(
        "/sources/ss_sheet",
        data={"display_name__val": "val", "data_type__val": "text", "include__val": "on"},
        follow_redirects=False,
    )
...
def test_corrupt_parquet_upload_is_friendly(client):
    ...
    resp = client.post(
        ...
        files={"file": ("bad.parquet", io.BytesIO(b"garbage parquet"), "application/octet-stream")},
        follow_redirects=False,
    )
    assert resp.status_code == 200, f"Expected 200 friendly page, got {resp.status_code}"
    body = resp.text.lower()
...
def test_history_tab_shows_secrets_warning_for_url_source(client):
    ...
    assert "plaintext" in resp.text, "Expected the plaintext-credentials warning in the History tab"

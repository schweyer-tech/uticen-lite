...
def test_save_pure_pipeline_derives_sources_and_compiles_to_rule_spec(client):
    _make_source(
        client, "accounts", b"account_id,is_active,is_privileged\nA1,true,true\nA2,false,true\n"
    )
    graph = {
        "nodes": [
            {"id": "imp", "type": "import", "source_id": "accounts", "narrative": "All accounts"},
            {
                "id": "tst",
                "type": "test",
                "inputs": ["imp"],
...
def test_save_cross_source_pipeline_compiles_to_test_code(client):
    _make_source(
        client, "access_accounts", b"account_id,employee_id,is_active\nA1,E1,true\nA2,E2,true\n"
    )
...
def test_save_rejects_custom_node_that_reads_a_file(client):
    ...
    resp = _post_pipeline(client, "bad1", _custom_pipeline("rows = open('/etc/passwd').read()"))
...
def test_save_rejects_custom_node_using_read_csv(client):
    _make_source(client, "journal_entries", b"entry_id,amount\nE1,100\n")
    resp = _post_pipeline(client, "bad2", _custom_pipeline("rows = pd.read_csv('/secret.csv')"))
    assert resp.status_code == 422
...
def test_save_rejects_custom_node_using_dunder_import(client):
    _make_source(client, "journal_entries", b"entry_id,amount\nE1,100\n")
    resp = _post_pipeline(client, "bad3", _custom_pipeline("m = __import__('os')\nrows = rows"))
    assert resp.status_code == 422
...
def test_save_rejects_custom_node_using_eval(client):
    _make_source(client, "journal_entries", b"entry_id,amount\nE1,100\n")
    resp = _post_pipeline(client, "bad4", _custom_pipeline("rows = eval('rows')"))
    assert resp.status_code == 422
...
def test_save_accepts_clean_custom_transform_node(client):
    _make_source(client, "journal_entries", b"entry_id,amount\nE1,100\nE2,50\n")
    graph = _custom_pipeline("rows = rows[rows['amount'].astype(float) >= 100]")
...
def test_save_single_import_test_with_not_exists_in_binds_both_sources(client):
    ...
    _make_source(client, "active_users", b"user_id,name\nU1,Alice\nU2,Bob\nU3,Carol\n")
    # Source B: the reference set (referenced only by not_exists_in, no Import node)
    _make_source(client, "terminated_users", b"user_id,reason\nU3,resigned\n")
    ...
    assert c is not None
    assert "active_users" in c["source_ids"], "Import source must be bound"
...
def test_run_with_not_exists_in_condition_succeeds_without_unknown_source_error(client):
    ...
    """
    # Seed both sources.
    _make_source(client, "active_users2", b"user_id,name\nU1,Alice\nU2,Bob\nU3,Carol\n")
    _make_source(client, "terminated_users2", b"user_id,reason\nU3,resigned\n")
...
def test_save_pipeline_with_blank_condition_placeholder_does_not_500(client):
    ...
    """
    _make_source(client, "blank_cond_src", b"emp_id,status\nE1,active\nE2,terminated\n")
    _make_source(client, "blank_cond_src2", b"emp_id,dept\nE1,eng\nE2,hr\n")
...
def _make_simple_graph() -> dict:
    ...
    return {
        "nodes": [
            {"id": "imp", "type": "import", "source_id": "acct_as", "narrative": "All accounts"},
            {
                "id": "tst",
                "type": "test",
                "inputs": ["imp"],
...
def test_autosave_returns_fragment_not_redirect(client):
    ...
    _make_source(client, "acct_as", b"account_id,is_active\nA1,true\nA2,false\n")
    client.post(
        "/controls",
        data={
            "id": "as1",
    ...
    assert resp.status_code == 200, f"autosave should return 200 fragment, got {resp.status_code}"
...
def test_autosave_validation_error_returns_422_not_redirect(client):
    ...
    assert resp.status_code == 422, f"autosave error should return 422, got {resp.status_code}"
    assert _OFFRAMP_STABLE in resp.text

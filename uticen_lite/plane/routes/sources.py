...
_SOURCE_NEW_TEMPLATE = "source_new.html"
_SOURCES_PATH = "/sources"
...
# ---- route implementations (module-level so each stays small; the route
# wrappers in register() only wire dependency injection and delegate here) ----

def _render_list_sources(
    templates: Jinja2Templates, request: Request, conn: sqlite3.Connection
) -> HTMLResponse:
...
def _render_new_source(
    templates: Jinja2Templates, request: Request, conn: sqlite3.Connection
) -> HTMLResponse:
    return templates.TemplateResponse(
        request,
        _SOURCE_NEW_TEMPLATE,
...
def _render_new_source_from_url(templates: Jinja2Templates, request: Request) -> HTMLResponse:
    return templates.TemplateResponse(
        request, _SOURCE_NEW_TEMPLATE,
...
def _render_edit_source(
    templates: Jinja2Templates, request: Request, source_id: str, conn: sqlite3.Connection
) -> HTMLResponse:
    return templates.TemplateResponse(
        request,
        "source_edit.html",
...
async def _update_asof(request: Request, source_id: str, as_of_date: str) -> RedirectResponse:
    root = request.app.state.project_root
    conn = connect(root)
    try:
...
async def _save_source(request: Request, source_id: str) -> RedirectResponse:
    root = request.app.state.project_root
    conn = connect(root)
    try:
        ...
        if existing is None:
            return RedirectResponse(_SOURCES_PATH, status_code=303)
        key_columns = [
            k.strip()
...
async def _confirm_refresh(
    request: Request, source_id: str, pending: str, as_of_date: str
) -> RedirectResponse:
...
async def _cancel_refresh(request: Request, source_id: str, pending: str) -> RedirectResponse:
...
async def _refetch_source(
    templates: Jinja2Templates, request: Request, source_id: str
) -> HTMLResponse | RedirectResponse:
...
def register(
    ...
    def list_sources(
        ...
        return _render_list_sources(templates, request, conn)
    ...
    def new_source(
        ...
        return _render_new_source(templates, request, conn)
    ...
    async def create_source(
        ...
        return await _create_source(templates, request, source_id, as_of_date, sheet, file)

    @app.get("/sources/from-url", response_class=HTMLResponse)
    def new_source_from_url(request: Request) -> HTMLResponse:
        return _render_new_source_from_url(templates, request)
    ...
    async def create_source_from_url(
        ...
        record_path: Annotated[str, Form()] = "",
        as_of_date: Annotated[str, Form()] = "",
    ) -> HTMLResponse | RedirectResponse:
        return await _create_source_from_url(
            templates, request, source_id, url, headers, record_path, as_of_date
        )
    ...
    def edit_source(
        ...
        return _render_edit_source(templates, request, source_id, conn)
    ...
    def source_data(
        ...
        return _render_source_data(templates, request, source_id, page, conn)
    ...
    def source_history(
        ...
        return _render_source_history(templates, request, source_id, conn)
    ...
    async def update_asof(
        ...
        return await _update_asof(request, source_id, as_of_date)
    ...
    async def save_source(
        ...
        return await _save_source(request, source_id)
    ...
    async def refresh_source(
        ...
        return await _refresh_source(templates, request, source_id, file, as_of_date)
    ...
    async def confirm_refresh(
        ...
        return await _confirm_refresh(request, source_id, pending, as_of_date)
    ...
    async def cancel_refresh(
        ...
        return await _cancel_refresh(request, source_id, pending)
    ...
    async def refetch_source(source_id: str, request: Request) -> HTMLResponse | RedirectResponse:
        return await _refetch_source(templates, request, source_id)

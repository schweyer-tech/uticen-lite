...
The DataFrame is accessed solely through the :class:`~uticen_lite.model.population.Population`
...
from uticen_lite.model.control import ControlDef, SourceBinding
...
from uticen_lite.model.workpaper import MAX_SAMPLE_ROWS, DataSample
...
def _clean_traceback_summary(exc: BaseException) -> str:
    ...
    SDK-internal frames (any frame whose filename contains 'uticen_lite'
    ...
    while current is not None:
        ...
        if "uticen_lite" not in filename and "site-packages" not in filename:
            user_frames.append(
...
def run_control(
    ...
    ----------
    control:
        The parsed :class:`~uticen_lite.model.control.ControlDef` whose
    ...
    sources:
        ...
        :func:`~uticen_lite.project.loader.load_sources`).  The runner
    ...
    RunRecord
        ...
        :meth:`~uticen_lite.model.violation.Violation.from_raw`), and
...
def collect_data_samples(
    ...
    Used by ``uticen-lite run`` to embed an interactive data table in the HTML

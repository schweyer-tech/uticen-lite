"""``uticen-lite build`` subcommand — assemble a versioned import-bundle zip.

Usage
-----
    uticen-lite build [dir] [--out import-bundle.zip] [--at <iso8601>]
...
from uticen_lite.bundle import BundleError
from uticen_lite.cli._store_guard import check_store
...
def build_cmd(args: argparse.Namespace) -> int:
    """Handle ``uticen-lite build [dir] [--out <path>] [--at <iso8601>]``."""
    ...
    if not runs_by_control:
        print(
            "ERROR: No runs found in store — run `uticen-lite run` first, "

...
from uticen_lite.model.control import ControlDef, SourceBinding, Threshold
...
from uticen_lite.pipeline.model import Node, Pipeline, parse_pipeline
...
def run_control_in_store(
    ...
    Steps:
    1. Load the :class:`~uticen_lite.project.discovery.Project` from *conn*.
    2. Locate the control by *control_id*.
    3. Execute the control via :func:`~uticen_lite.runner.execute.run_control`.
    4. Persist the :class:`~uticen_lite.model.run.RunRecord` via
       :func:`~uticen_lite.store.repo.insert_run`.
    5. Collect data samples and assemble a
       :class:`~uticen_lite.model.workpaper.Workpaper`.
    6. Write ``target/workpapers/<id>.html``, ``target/workpapers/<id>.md``, and
       ``target/evidence/<id>-violations.json`` under *root*.
    7. Return the :class:`~uticen_lite.model.run.RunRecord`.

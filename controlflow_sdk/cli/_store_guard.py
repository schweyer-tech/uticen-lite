...
into an actionable message telling the user to run ``cflow import`` first.
"""
...
from controlflow_sdk.store.db import DB_FILENAME
...
def check_store(conn: sqlite3.Connection, root: Path) -> bool:
    ...
    print(
        ...
        f"(expected {DB_FILENAME}). Run `cflow import <yaml-project>` first.",
        file=sys.stderr,
    )
    return False

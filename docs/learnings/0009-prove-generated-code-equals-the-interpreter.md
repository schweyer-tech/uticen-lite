...
Two surfaces generate Python that the ControlFlow app later re-runs from the bundle: the rule grammar
...
- `controlflow_sdk/rules/render_rule.py` (`_mask_expr`, `frame=` param — single source of truth).
- `controlflow_sdk/rules/evaluate.py` (the canonical interpreter the generated code must match).

...
| [0003](0003-hatch-wheel-no-force-include-package-data.md) | 2026-06-19 | packaging | Hatchling `packages = ["pkg"]` already ships package-internal data files; `force-include`-ing a path already inside the package fails the wheel build (duplicate archive path). Force-include only out-of-package/VCS-ignored files; verify packaging by BUILDING the wheel, not by parsing pyproject. | active |

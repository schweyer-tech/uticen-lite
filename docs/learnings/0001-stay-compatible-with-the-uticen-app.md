...
date: 2026-06-19
area: contract
tags: [bundle, contract, uticen-app, compatibility, trust-boundary, cardinal]
status: active
supersedes: null
superseded_by: null
---

# Cardinal rule: keep the SDK bundle-compatible with the Uticen app — `contract/bundle.schema.json` is the binding contract

## Context

`uticen-lite` exists to feed the Uticen SaaS app: the SDK authors + runs full-population
...
- **Every bundle producer reuses `assemble_bundle` + `write_bundle` — never fork the shape.** The CLI
  (`uticen-lite build`) and the control-plane web export (`store/export_service.py:build_bundle`) are both
